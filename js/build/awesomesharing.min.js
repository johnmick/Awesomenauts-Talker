...
"400px";$("#PLAY_BUTTON").click(k.playSounds);$("#SHARE_BUTTON").click(function(){window.location.href="./index.htm"});j=document.getElementById("PHRASES_MESSAGE");for(var b=0;b<a.length;b++){var d=document.createElement("span");d.innerHTML=b===0?'"'+a[b]+" ":b+1===a.length?a[b]+'."':a[b]+" ";j.appendChild(d);i.push(d)}return n};n.highlightNextPhrase=function(){g++;if(g!==0){i[g-1].style.fontWeight="normal";i[g-1].style.color="#FFFFFF"}if(g!==i.length){i[g].style.fontWeight="bold";i[g].style.color=
"#D2FF60"}};n.reset=function(c){g=-1;if(c!==true){i[0].style.fontWeight="bold";i[0].style.color="#D2FF60"}else{i[i.length-1].style.fontWeight="normal";i[i.length-1].style.color="#FFFFFF"}}})();var o;(function(){function i(a){function b(q){var l={},m;for(m in q)l[m]=q[m];l.onload=AwesomeLoading.somethingLoaded;l.autoLoad=true;return l}if(a.UI!==undefined){j.UI={};for(var d=a.UI.SOUNDS,e=0;e<d.length;e++){var f=d[e];j.UI[f.id]=soundManager.createSound(b(f))}g=j.UI.UI_TITLE_MUSIC}for(var h in a.CHARACTERS){d=
a.CHARACTERS[h].PHRASES;j[h]={};e=a.CHARACTERS[h].THEME_SONG;e.autoLoad=true;e.onload=c===true?k.checkLoadStatus:AwesomeLoading.somethingLoaded;j[h].THEME=soundManager.createSound(e);for(e=0;e<d.length;e++){f=d[e];var p=f.TXT.replace("'","");if(c===true)j[h][p]=soundManager.createSound({id:h+"_"+p,url:f.SRC,autoLoad:true,volume:100,onload:k.checkLoadStatus,onfinish:function(){}});else j[h][f.TXT]=soundManager.createSound({id:h+"_"+p,url:f.SRC,autoLoad:true,volume:100,onload:AwesomeLoading.somethingLoaded,
onfinish:function(){AwesomePhrases.unhighlight(this.id.split("_")[0],this.id.split("_")[1])}})}}}var j={},g=undefined,c=false;o=function(a,b){c=b;soundManager.setup({onready:function(){i(a)}});return o};o.switchMusic=function(a){g!==undefined&&g.stop();g=j[a].THEME;g.play()};o.play=function(a,b){function d(){console.log("Unable to Find Sounds Reference to:",a,b)}if(j[a]!==undefined)j[a][b]!==undefined?j[a][b].play():d();else d()}})();soundManager.setup({url:"./swf/",useHighPerformance:true});(function(){function i(){var c=
decodeURI(window.location.href);c=c.slice(c.indexOf("?")+1).split("&");for(var a={},b={readC:function(h){return h},readP:function(h){return h.split(",")},readT:function(h){return h.split(",")}},d=0;d<c.length;d++){var e=c[d].substring(0,1).toUpperCase(),f="read"+e;if(b[f]!==undefined)a[e]=b[f](c[d].substring(2,c[d].length))}return a}function j(c,a){if(c.C===undefined||c.C===null)return"The character name parameter, 'c={character_name}', is missing";if(a.CHARACTERS[c.C]===undefined){var b="The character name, '"+
c.C+"', is not one of the valid characters:<br/>",d;for(d in a.CHARACTERS)b+="<br/>"+d;return b}if(c.P===undefined)return"Missing phrases parameter, p={phrases_list}, where<br/>'phrases_list' is a comma delimited string of phrases";else{d=c.P;var e=a.CHARACTERS[c.C].PHRASES;for(b=d.length-1;b>-1;b--){for(var f=false,h=e.length-1;h>-1;h--)if(e[h].TXT===d[b]){f=true;break}if(f===false)return"Invalid Phrase, '"+d[b]+"', Specified<br/><br/>No configuration data available for this phrase and character."}}if(c.T===
